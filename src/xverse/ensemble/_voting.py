...
class VotingSelector(BaseEstimator, TransformerMixin):
    ...
    def recursive_feature_elimination(self, X, y, name):
        ...
        output = self.create_dataset(
            rfe_dict_mapping.values(), name, rfe_dict_mapping.keys()
        )
        return output
    ...
    def fit(self, X, y):
        ...
        if self.exclude_features:
            self.use_features = list(
                set(self.use_features) - set(self.exclude_features)
            )

        # raise error if there is no features to select
        if not list(self.use_features):
            raise ValueError(
                "No input feature list provided. Please provide a list of features or \
    ...
    def fill_numeric_missing_values(self, X, num_miss_present):
        ...
        if self.numerical_missing_values == "median":
            ...
            raise ValueError(
                "Error while handling imputation for numerical values. \
    ...
    def train(self, X, y):
        ...
        if self.selection_techniques == "all":
            ...
            if not isinstance(self.selection_techniques, list):
                raise ValueError(
                    "The selection techniques provided should be list. \
                                But the input provided is "
                    + str(type(self.selection_techniques))
                )

            if not self.selection_techniques:
                raise ValueError(
                    "The selection technique list is empty. Please provide a list of techniques."
                )
        ...
        if self.handle_category == "woe":
            transformed_X, self.mapping, iv_df = self.woe_information_value(
                X, y
            )  # woe transformed_X
            ...
            le_fit = (
                ...
                .fillna("NA")
                .apply(lambda x: mapping[x.name].fit_transform(x))
            )
            ...
            raise ValueError(
                "Error while handling categorical values. \
        ...
        X = pd.concat(
            [transformed_X[self.categorical_features], X[self.numerical_features]],
            axis=1,
        )
        ...
        if output_columns:  # check if any feature selection technique is executed above
            final_results = reduce(
                lambda left, right: pd.merge(
                    left, right, on="Variable_Name", how="outer"
                ),
                dfs,
            )
            ...
            for i in output_columns:
                ...
                top_n_value_index = (
                    final_results[i].abs().nlargest(self.no_of_features).index
                )
            ...
            raise ValueError(
                "Please provide a valid selection technique. \
    ...
    def transform(self, X):
        ...
        if not self.mapping:
            raise ValueError(
                "Mapping variable is not present. \
        ...
        # select features
        self.use_features = self.feature_votes_[
            self.feature_votes_["Votes"] >= self.minimum_votes
        ]["Variable_Name"]
        ...
        if self.mapping:
            for k in X.columns:
                if isinstance(self.mapping[k], dict):
                ...
                elif isinstance(self.mapping[k], int) or isinstance(
                    self.mapping[k], float
                ):
                    ...
                    raise ValueError(
                        (
                            "Value provided for input variable mapping is not valid. \
