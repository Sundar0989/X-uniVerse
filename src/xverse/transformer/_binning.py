...
class MonotonicBinning(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        ...
        print("initiated monotonic binning ")
    ...
    def train(self, X, y):
        ...
        if fitted:  # fit is required
            ...
            if len(bins_X_grouped) == 1:
                bins = pd.DataFrame.quantile(
                    pd.DataFrame(X), np.linspace(0, 1, force_bins)
                )
                # algos.quantile(X, np.linspace(0, 1, force_bins))
        ...
        print(f"bins - {bins} & mapped to {mapping}")
